...
function PollarEmailLogin() {
  ...
  const [syncAttemptId, setSyncAttemptId] = useState<string | null>(null);
  const [pulsoCode, setPulsoCode] = useState("");
  const [pollarDisplayName, setPollarDisplayName] = useState("");
  ...
  async function goToPanel(code?: string) {
    setSyncing(true);
    setSyncError(null);
    const result = await syncPollarToPulso(getClient, {
      emailHint: email,
      attemptId: syncAttemptId || undefined,
      code,
      displayName: pollarDisplayName || undefined,
    });
    setSyncing(false);
    if (result.needsCode && result.attemptId) {
      setSyncAttemptId(result.attemptId);
      setPollarDisplayName(result.displayName || "");
      return;
    }
    if (!result.ok) {
    ...
    if (syncAttemptId) {
      return (
        <section className="space-y-3 rounded-xl border border-border bg-surface p-4">
          <p className="text-sm font-semibold tracking-wide text-primary">VERIFICACIÓN PULSO</p>
          <h2 className="text-lg font-semibold text-text">Confirma tu correo</h2>
          <p className="text-sm text-text-secondary">
            Enviamos un segundo código de seguridad a <strong className="text-text">{email}</strong>. Así comprobamos que el correo y la cartera Pollar pertenecen a la misma persona.
          </p>
          <label className="block text-sm text-text-secondary" htmlFor="pulso-otp">Código de 6 dígitos</label>
          <input id="pulso-otp" type="text" inputMode="numeric" autoComplete="one-time-code" maxLength={6} value={pulsoCode} onChange={(event) => setPulsoCode(event.target.value.replace(/\D/g, "").slice(0, 6))} className="min-h-[52px] w-full rounded-xl border border-border px-3 text-center text-2xl tracking-[0.4em] text-text" placeholder="••••••" disabled={syncing} />
          {syncError ? <p className="text-danger" role="alert">{syncError}</p> : null}
          <button type="button" disabled={syncing || pulsoCode.length !== 6} onClick={() => void goToPanel(pulsoCode)} className="flex min-h-[52px] w-full items-center justify-center rounded-xl bg-primary px-4 font-medium text-white disabled:opacity-60">
            {syncing ? "Comprobando…" : "Confirmar y entrar"}
          </button>
        </section>
      );
    }
