...
Pulso propone que una persona en casa pueda pedir ayuda a su red familiar mediante **dos pulsaciones físicas** de un botón ESP32 conectado a Wi-Fi. Una pulsación aislada no genera alerta. La familia vería el incidente, confirmaría quién responde y tendría un historial legible. Un chequeo diario voluntario complementaría la conversación de cuidado. La experiencia web y sus avisos son distintos de una solicitud al 911; ante una urgencia se debe llamar al número local de emergencias.

El repositorio contiene una landing local, una vista previa web, detección local de doble pulsación en firmware y la interfaz prevista de un contrato Soroban. **Aún no existe un flujo completo de alerta en producción.** La demostración final del proyecto usará una ESP32 real; su integración queda para después de cerrar la landing. La hipótesis de Pulso es que una coordinación más clara puede reducir incertidumbre en la familia; eso debe medirse en un piloto y no se presenta como resultado probado.
...
**Persona en casa:** dos pulsaciones separadas del botón físico dentro de una ventana inicial de 3 segundos, panel con texto grande y contactos elegidos. Una sola pulsación expira sin efecto de alerta. La ventana se validará con personas mayores y cuidadoras.

**Familia/cuidadora:** aviso por correo, apertura del caso, confirmación de responsabilidad y estado compartido. La confirmación familiar ocurre **después** de los dos toques físicos; no forma parte de ellos.
...
3. Primera pulsación: el ESP32 espera una segunda, sin crear alerta. Segunda pulsación separada dentro de 3 segundos: evento HTTPS autenticado, `event_id` e idempotencia. Si expira la ventana, se reinicia sin evento.
...
La fiabilidad exige probar pulsación aislada, doble pulsación válida, expiración de la ventana, rebote mecánico, entrega de correos, reintentos, confirmación, falso positivo, duplicados y caída de internet. Ninguna interfaz debe mostrar «familia avisada» antes de confirmar entrega del proveedor; aun entregado, correo no garantiza que alguien lo leyó.
...
Piloto inicial con **2–5 usuarios y familiares voluntarios**. Medir: porcentaje de dispositivos vinculados; dobles pulsaciones de prueba recibidas; pulsaciones únicas descartadas; intentos fallidos por límite de tiempo; correos aceptados por proveedor; tiempo hasta apertura y confirmación; duplicados; falsos positivos; comprensión del estado por personas mayores; finalización del chequeo; satisfacción y fricciones. Separar simulacros de incidentes reales. Criterio mínimo antes de confiar en el sistema: pruebas completas, canal alternativo documentado y protocolo claro de que el familiar llama al 911.
...
Mostrar la P de Pulso y el lema; ejecutar una sola pulsación simulada y demostrar que no se genera alerta; hacer el segundo toque dentro de 3 segundos y luego una confirmación familiar separada; después enseñar el firmware detectando la doble pulsación local. Separar con precisión cada pieza funcionando de la arquitectura futura. Explicar por qué Stellar guarda una prueba de integridad y no información clínica. Cerrar con la hipótesis que el piloto comprobará: si la familia entiende más rápido quién actúa.
